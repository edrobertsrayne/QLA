...
- **Exam specifications are fetched once, not re-uploaded per assessment**: user supplies board + subject + tier + a URL to the official spec PDF; the server fetches and parses it into spec codes (reusing the same extraction approach as markscheme/paper parsing), caching the result keyed by board + subject + tier + version/issue. A spec revision is a new cache entry, not an overwrite.
- **Single-user, no auth.** Multi-teacher use is explicitly out of scope (see below).

## Phase 1 — Parse

Upload a markscheme + assessment paper (PDF) → configurable OpenRouter model, called via the server proxy → structured JSON breakdown of questions mapped to spec codes, checked against a cached exam specification. No UI polish beyond a functional single page — this phase is about the pipeline working, not how it looks.
...
- Where the parsed breakdown and the cached spec live (browser storage vs. server-side file cache — this data isn't student data, so Q11's "server persists nothing" doesn't automatically decide it)
