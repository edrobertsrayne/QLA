<!--
	PROTOTYPE — copy of #27's winning Variant D grid, reused as the marks surface for wayfinder #28.
	Header copy stripped; an optional `footer` snippet renders a <tfoot> (Variant B's summary row) and
	the grid scrolls inside a bounded box so that footer can stay pinned to the bottom.
-->
<script lang="ts">
	...
	let {
		engine,
		footer
	}: { engine: GridEngine; footer?: Snippet<[{ groupStarts: Set<number> }]> } = $props();
...
<div class="bg-background">
	...
	<div
		class="mx-4 max-h-[calc(100vh-11rem)] overflow-auto rounded-lg border border-border"
		role="grid"
	>
